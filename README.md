...
3. **Party debate** — five party agents (KO, PiS, TD, Konfederacja, Lewica) argue from their real ideology across a first and second reading.
...
Marszałek (orchestrator skill) ── delegate_task(parallel) ──► 19 ministry skills ──► PageIndex (legal RAG)
        │                                                                                  ▲
        ├─ first + second reading ──► 5 party skills (KO/PiS/TD/Konfederacja/Lewica) ──────┘ (cite sources)
