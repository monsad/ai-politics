...
def test_party_divergence_profile_ko_vs_konfederacja():
    """
    KO and Konfederacja must have visibly opposing positions on flat tax.
    Verified by text-level check — no LLM call needed.
    """
    ko = (SKILLS_DIR / "party-ko" / "SKILL.md").read_text(encoding="utf-8").lower()
    konfederacja = (SKILLS_DIR / "party-konfederacja" / "SKILL.md").read_text(encoding="utf-8").lower()

    # Both must mention flat tax
    assert "flat tax" in ko, "party-ko SKILL.md missing 'flat tax' position"
    assert "flat tax" in konfederacja, "party-konfederacja SKILL.md missing 'flat tax' position"
    ...
    # Konfederacja supports flat tax
    ...
    assert konf_supports, (
        f"party-konfederacja flat tax section should contain support language. Got: {konf_context!r}"
    )
