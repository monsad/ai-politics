...
PARTIES = ["KO", "PiS", "TD", "Konfederacja", "Lewica"]
...
def _split_party_speeches(section_text: str, phase: str) -> list[dict]:
    ...
    Speeches start with patterns like `**KO (157 seats):**` or `**Lewica rebuttal:**`.
    """
    out: list[dict] = []
    pattern = re.compile(
        r"\*\*\s*(KO|PiS|TD|Konfederacja|Lewica)\b[^*]*\*\*",
        re.IGNORECASE,
    )
